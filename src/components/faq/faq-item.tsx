    ...
    <div className="nt-faq-item">
      <h2>
        <button
          type="button"
          onClick={onToggle}
          className="nt-w-full nt-flex nt-items-center nt-justify-between nt-gap-4 nt-text-left nt-py-8 nt-text-white nt-font-normal nt-text-h4"
          aria-expanded={isOpen}
          aria-controls={`faq-answer-${item.id}`}
          id={`faq-question-${item.id}`}
        >
          <span className="nt-flex-1 nt-min-w-0">{item.question}</span>
          <RiAddLine
